...
async function detectPackageManager(
  ...
  if (fileExists(path.join(projectPath, "pnpm-lock.yaml")))
    preferred.push("pnpm");
  if (fileExists(path.join(projectPath, "package-lock.json")))
    preferred.push("npm");
  ...
  if (preferred.length === 0) {
    ...
    throw new Error(
      "No package manager found. Install pnpm or Node.js (npm) to run preview.",
    );
  }
...
function convertNextConfigTsToMjs(ts: string): string {
  ...
  s = s.replace(
    /import\s+type\s*\{\s*NextConfig\s*\}\s*from\s*['"]next['"]\s*;?\s*/g,
    "",
  );
  s = s.replace(
    /import\s*\{\s*NextConfig\s*\}\s*from\s*['"]next['"]\s*;?\s*/g,
    "",
  );
...
function projectUsesTailwindV4(
  globalsCss: string | null,
  postcssCfg: string | null,
): boolean {
 ...
 */
async function repairProjectForPreview(
  inst: Instance,
  projectPath: string,
): Promise<{ reinstallDeps: boolean }> {
  ...
  if (fileExists(nextConfigTs)) {
    if (!fileExists(nextConfigMjs) && !fileExists(nextConfigJs)) {
      pushLogLine(
        inst,
        "🛠 Repair: Migrating unsupported next.config.ts → next.config.mjs",
      );
      ...
      pushLogLine(
        inst,
        "🛠 Repair: Removing unsupported next.config.ts (keeping existing JS/MJS config)",
      );
    }
    ...
    if (pkg) {
      ...
      if (postcssPath) {
        if (!/@tailwindcss\/postcss/.test(postcssCfg ?? "")) {
          pushLogLine(
            inst,
            `🛠 Repair: Updating PostCSS config to Tailwind v4 (@tailwindcss/postcss): ${path.relative(projectPath, postcssPath)}`,
          );
        ...
        pushLogLine(
          inst,
          "🛠 Repair: Adding missing PostCSS config for Tailwind v4 (postcss.config.mjs)",
        );
      ...
      if (
        pkg.dependencies["tw-animate-css"] == null &&
        pkg.devDependencies["tw-animate-css"] == null
      ) {
      ...
      if (
        pkg.devDependencies.tailwindcss == null &&
        pkg.dependencies.tailwindcss == null
      ) {
        pkg.devDependencies.tailwindcss = TAILWIND_V4;
        changed = true;
      } else if (
        typeof pkg.devDependencies.tailwindcss === "string" &&
        pkg.devDependencies.tailwindcss.startsWith("^3")
      ) {
        pkg.devDependencies.tailwindcss = TAILWIND_V4;
        changed = true;
      }
      ...
      if (
        pkg.devDependencies["@tailwindcss/postcss"] == null &&
        pkg.dependencies["@tailwindcss/postcss"] == null
      ) {
      ...
      if (changed) {
        pushLogLine(
          inst,
          "🛠 Repair: Ensuring Tailwind v4 dependencies are present in package.json",
        );
...

  {
    ...
    if (pkg) {
      ...
      if (changed) {
        pushLogLine(
          inst,
          "🛠 Repair: Adding missing Next.js dependencies (next/react/react-dom) to package.json",
        );
        await writeJson(pkgPath, pkg);
        reinstallDeps = true;
      }
...

  {
    ...
    const pkg = await readJson<any>(pkgPath);
    const hasNextPwa = !!(
      pkg?.dependencies?.["next-pwa"] ?? pkg?.devDependencies?.["next-pwa"]
    );
    ...
    if (cfgPath) {
      ...
      if (cfg && /next-pwa/.test(cfg) && !hasNextPwa) {
        pushLogLine(
          inst,
          "🛠 Repair: Disabling next-pwa in next.config (dependency missing)",
        );
  ...
  for (const layoutPath of layoutCandidates) {
    ...
    if (looksWrong) {
      pushLogLine(
        inst,
        `🛠 Repair: Rewriting invalid App Router layout: ${path.relative(projectPath, layoutPath)}`,
      );
...
`;
    ...
    if (
      ...
      (src.includes("Geist") || src.includes("Geist_Mono"))
    ) {
      pushLogLine(
        inst,
        `🛠 Repair: Rewriting Geist font import to Inter/Roboto_Mono: ${path.relative(projectPath, layoutPath)}`,
      );
  ...
  for (const pagePath of pageCandidates) {
    ...
    const hasMetadataExport =
      /export\s+const\s+metadata/.test(src) ||
    ...
    const usesClientOnly =
      /use(State|Effect|LayoutEffect|Memo|Callback|Ref|Reducer|Transition|DeferredValue|Optimistic|SyncExternalStore|Id)/.test(
        next,
      ) ||
    ...
    if (!usesClientOnly) {
      ...
      pushLogLine(
        inst,
        `Repair: Removing unnecessary 'use client' from ${path.relative(projectPath, pagePath)} (to allow metadata export)`,
      );
    } else {
      next = stripClientMetadataExports(next);
      pushLogLine(
        inst,
        `Repair: Removing metadata export from client page ${path.relative(projectPath, pagePath)} (Next.js disallows this)`,
      );
    }
  ...
  if (!fileExists(buttonAbs)) {
    ...
    if (usesButton) {
      pushLogLine(
        inst,
        `🛠 Repair: Adding missing UI button component (${buttonRel})`,
      );
...
`;
  ...
  for (const pagePath of pageCandidates) {
    ...
    if (next !== src) {
      pushLogLine(
        inst,
        `🛠 Repair: Fixing bad Button import in ${path.relative(projectPath, pagePath)}`,
      );
      await writeText(pagePath, next);
    }
  }
...
class PreviewManagerImpl implements PreviewManager {
  ...
  async start(opts: PreviewStartOptions): Promise<PreviewStatus> {
    ...
    if (inst?.state === "running" && inst.proc && !inst.proc.killed)
      return toStatus(inst);
    if (!inst) {
